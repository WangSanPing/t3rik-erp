    ...
    public LoginUser getLoginUser(HttpServletRequest request) {
        ...
        if (StringUtils.isNotEmpty(token)) {
            try {
                ...
                JSONObject json = (JSONObject)redisCache.getCacheObject(userKey);
                return JSON.parseObject(json.toJSONString(),LoginUser.class);
            } catch (Exception e) {
                log.error("解析登录用户失败，失败异常:{}", e.getMessage());
            }
        }
        return null;
    }
